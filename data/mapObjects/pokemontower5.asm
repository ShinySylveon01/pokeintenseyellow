PokemonTower5Object:
	db $1 ; border block

	db 2 ; warps
	warp 3, 9, 0, POKEMONTOWER_4
	warp 18, 9, 0, POKEMONTOWER_6

	db 0 ; signs

	db 6 ; objects
	object SPRITE_MEDIUM, 12, 8, STAY, NONE, 1 ; person
	object SPRITE_MEDIUM, 12, 11, STAY, RIGHT, 2, OPP_CHANNELER, 14
	object SPRITE_MEDIUM, 11,  4, STAY, DOWN, 3, OPP_CHANNELER, 16
	object SPRITE_MEDIUM, 6, 10, STAY, RIGHT, 4, OPP_CHANNELER, 17
	object SPRITE_MEDIUM,  7, 14, STAY, RIGHT, 5, OPP_CHANNELER, 18
	object SPRITE_BALL, 10, 16, STAY, NONE, 6, NUGGET

	; warp-to
	warp_to 3, 9, POKEMONTOWER_5_WIDTH ; POKEMON_TOWER_4F
	warp_to 18, 9, POKEMONTOWER_5_WIDTH ; POKEMON_TOWER_6F
